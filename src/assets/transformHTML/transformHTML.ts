...
import { customIndexOf, customSubstring } from "../customFn";
...
// };
export const transformHTML = (html: string, uniqueMap:TagUniqueMap) => {

    for(const [originalTag,customTag] of Object.entries(uniqueMap)){
        let startIndex = 0;
        while((startIndex = customIndexOf(html,`<${originalTag}>`,startIndex))!==-1){
            const endOfTag = customIndexOf(html,">",startIndex);
            if(endOfTag === -1) break;

            html = customSubstring(html,startIndex) + `<${customTag}` + customSubstring(html,endOfTag);
            startIndex +=customTag.length + 1;
        }
        startIndex =0;

        while((startIndex=customIndexOf(html,`</${originalTag}>`,startIndex))!==-1){
            html = customSubstring(html,0,startIndex)+ `</${customTag}>` + customSubstring(html,startIndex+originalTag.length+3)
            startIndex += customTag.length+3;
        }
    }
    return html;
};
